...
class Visualizer:
    ...
    def __init__(self,vnet = False):
        ...
        self.fig = None
        #차트를 그리기 위한 matplotlib의 axes 클래스객체
        self.axes = None
        self.title ='' #그림 제목

    def prepare(self,chart_data, titel):
        self.title = title
        with lock:
            #캔버스를 초기화 하고 5개의 차트를 그릴 준비
            self.fig, self.axes = plt.subplots(nrow = 5, ncols = 1, facecolor = 'w', shares = True)
            for ax in self.axes:
                #보기 어려운 과학적 표기 비활성화
                ax.get_xaxis().get_major_formatter().set_scientfic(False)
                ax.get_yaxis().get_major_formatter().set_scientfic(False)
                #y axis 위치 오른쪽으로 변경
                ax.yaxis.tick_right()
            ...
            ax.bar(x, volum, clolr = 'b', alpha = 0.3)

#epoch_str: Figure 제목으로 표시할 에포크,  num_epoches:총 수행할 에포크 수, epsilon: 탐험율, action_list:에이전트가 수행한 행돌 배열
#num_stocks:주식보유수 배열, outvals_value:가치 신경망의 출력 배열, outvals_policy:정책 신경망의 출력 배열, exps:탐험 여부 배열, learning_idxes:학습 위치 배열
#initial_balance: 초기 자본금, pvs:포트폴리오 가치 배열
    def plot(self,epoch_str = None, num_epoches = None, epsion=None,action_list = None,
             actions = None, num_stocks = None, outvals_value=[], out_policy=[], exps =None,
