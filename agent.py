...
class Agent:
    #에이전트 상태가 구성하는 값 개수
    STATE_DIM = 2 #주식보유비율 ,포트폴리오 가치 비율
    ...
    def __init__(self, environment, min_trading_unit = 1, max_trading_unit=2,delayed_reward_threshold = .05):
        ...
        #최소매매 단위,최대매매단위, 지연보상 임계치
        self.min_trading_unit = min_trading_unit  #최소 단일 거래단위
        ...
        self.blance = 0 #현재 현금 잔고
        self.num_stocks = 0 #보유 주식 수
        #PV= blance + num_stocks *{현재 주식 가격)
        ...
        self.exploration_base = 0 #탐험 행동결정 기준
    ...
    def reset(self):
        self.blance = self.initial_balnce
        self.num_stocks = 0
    ...
    #decide_action() 입력으로 들어온 epsilon의 확률로 무작위로 행동을 결정하고 그렇지 않은 경우에신경망을 통해 행동을 결정합니다.
    def decide_action(self,pred_value, pred_policy, epsilon):
        ...
        if np.random.rand() < epsilon:
            exploration = True
            if np.random.rand() < self.exploration_base:   #exploration_base가 1에가까우면 매수, 0에가까우면 매도 탐험.
                action = self.ACTION_BUY
            else:
                action = np.random.randint(self.NUM_ACTIONS -1) +1   #0 매수, 1 매도
        else:
            exploration = False
            action = np.argmax(pred)

        confidence = .5
        if pred_policy is not None:   #정책 신경망의 출력
            confidence = pred[action]  #pred_policy로 행동 결정
        elif pred_value is not None:
            confidence = utils.sigmoid(pred[action])  #pred_value로 행동 결정

        return action, confidence, exploration



#결정한 행동을 수행할 수 있는지를 확인하기 위함.
    def validate_action(self,action):
        if action == Agent.ACTION_BUY:
            #적어도 1주를 살 수 있는지 확인
            if self.balance < self.environment.get_price() * (1 + self.TRADING_CHARGE0 ) * self.min_trading_unit:
                return False
...
#정책 신경망이 결정한 행동의 신뢰가 높을수록 매수 또는 매도하는 단위를 크게 정해줌
...
#에이전트가 결정한 행동을 수행합니다.
    def act(self,action,confidence):  #action: 탐험 또는 정책 신경망을 통해 결정한 행동으로 매수(0) 또는 매도91)를 의미, confidence는 정책 신경망을 통해 결정한 경우 결정한 행동에 대한 소프트맥스 확률값
